...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("ui_J") and (not is_attacking or combo_ready):
		...
		if attack_state == 0:
			...
			attack_state = 2
			emit_signal("punch_activated_p2", "state2_p2")  # Emite sinal para ativar colisões de punch1
		else:
			animation.play("punch3")
			attack_state = 0
			emit_signal("punch_activated_p2", "state3_p2")  # Emite sinal para ativar colisões de punch1
